...
/* 표를 읽지 않는 사람이 가져갈 결론. 세 문장을 넘기지 않는다. */
const TAKEAWAYS = [
  {
    kind: 'neutral',
    title: '코딩 기본기로는 고를 수 없다',
    body: '코드베이스 이해, 테스트, MCP 연동, 산출물 관리에서 네 도구의 판정이 같다. 23개 항목 중 5개가 그렇다. "어느 도구가 코드를 더 잘 쓰는가"로 도입을 결정하려는 시도는 근거를 만들기 어렵다.',
  },
  {
    kind: 'pro',
    title: 'Bob이 단독으로 앞서는 곳은 두 군데다',
    body: 'COBOL·PL/I·RPG·Java 전용 현대화 워크플로를 가진 유일한 도구이고, 리뷰 결과를 처리 상태가 남는 Findings 목록으로 관리하는 것도 Bob뿐이다. 여기에 행위 단위 비용 가시성과 팀·개인 지출 한도가 붙는다.',
  },
  {
    kind: 'con',
    title: 'Bob은 개발자 책상 위에서만 돈다',
    body: '실행 표면이 IDE와 터미널 둘뿐이다. 나머지 세 도구는 웹·모바일·Slack에서 작업을 받고, 예약 실행과 클라우드 에이전트로 사람이 없는 시간에도 일한다. 23개 항목 중 12개에서 Bob이 뒤진다.',
  },
];

/**
 * 표를 근거로 한 Bob 강점·약점 분석.
 * 장표에는 요약만 싣고, analysis.html에서 상세를 펼친다.
 * 모든 항목은 `evidence`로 표의 어느 행에서 나온 판단인지 밝힌다 — 근거 없는 총평을 막기 위함이다.
 */
const ANALYSIS = {
  boardLead:
    '위 표를 Bob 관점으로 정리하면 다음과 같습니다. 각 항목은 표의 특정 행에서 나온 판단이며, ' +
    '근거가 된 행을 함께 표시했습니다.',
  intro:
    '이 페이지는 새로운 정보를 담지 않는다. 23개 항목의 판정을 Bob 관점에서 다시 묶은 것이며, ' +
    '각 항목마다 근거가 된 행을 링크로 걸었다. 총평을 먼저 쓰고 근거를 나중에 찾는 순서를 피하려는 장치다. ' +
    '판정의 근거는 각 벤더의 공식 제품 문서를 우선으로 삼았고, Bob은 공식 docs 52개 페이지를 전수 확인했다.',

  strengths: [
    {
      title: '레거시 현대화는 대응물이 없다',
      summary: 'COBOL·PL/I·JCL·RPG·Java 전용 워크플로를 가진 유일한 도구',
      evidence: ['modernization'],
      body: [
        'Java 패키지에 네 개 워크플로가 문서화돼 있다 — 버전 업그레이드(AI 검증과 에이전트 수정 사이클), WebSphere에서 Liberty로의 리플랫포밍(IBM AMA 마이그레이션 플랜 기반), UI 현대화, 단위 테스트 생성. Z용(COBOL·PL/I·JCL)과 IBM i용(RPG·CL·DDS) 패키지는 별도로 제공된다.',
        '경쟁 도구 셋은 같은 작업을 범용 역량으로 수행한다. 할 수는 있지만 작업 분해와 검증 설계를 사용자가 맡는다. 범용 모델이 학습 데이터로만 아는 COBOL·PL/I·RPG에서는 이 격차가 특히 크다.',
        '우리 자산에 메인프레임이나 IBM i가 있다면 이 항목 하나만으로 검토를 시작할 이유가 된다. 반대로 자산이 전부 최신 스택이면 이 강점의 값은 0이다.',
      ],
      caveat:
        'Premium Package는 기본 구독에 포함되지 않는 별도 계약이다. 인용된 성과 수치(30일→3일 등)는 IBM이 고른 고객 사례이며 대상 코드베이스 상태에 크게 좌우된다. 그리고 현대화는 일회성 프로젝트이므로, 이 항목만으로 상시 개발 도구를 결정하면 프로젝트가 끝난 뒤 판단 근거가 남지 않는다.',
    },
    {
      title: '리뷰 결과가 상태를 가진 목록으로 남는다',
      summary: '/review가 Findings 패널에 심각도별로 누적 — 나머지는 코멘트로 흩어짐',
      evidence: ['code-review'],
      body: [
        '/review로 브랜치 간 diff를 분석해 이슈를 Findings 패널에 심각도별로 쌓고, 항목마다 위치·원인·영향을 붙인다. 무엇이 남았는지를 사람이 세지 않아도 된다.',
        '경쟁 도구 셋은 리뷰 결과를 PR 코멘트로 남긴다. 처리 상태 추적은 PR 도구 쪽 몫이 된다. 팀 규모가 커질수록 이 차이가 리뷰 누락으로 이어진다.',
      ],
      caveat:
        '이 우위는 판정 기준에 의존한다. 기준을 "사람이 잊어도 리뷰가 도는가"로 바꾸면 뒤집힌다 — Bob 문서는 리뷰 워크플로가 전적으로 IDE 안에서 돈다고 명시하고, 나머지 셋은 PR마다 자동으로 리뷰한다. Bob은 커밋 전 로컬 게이트이고 경쟁 도구는 머지 전 PR 게이트다. 둘 다 필요한 조직이라면 Bob만으로는 후자가 빈다.',
    },
    {
      title: '비용을 행위 단위로 보고 사전에 막는다',
      summary: 'Bobalytics 3층 구조 + 팀 예산 상한 + 개인 지출 한도',
      evidence: ['cost-visibility', 'pricing'],
      body: [
        '과금 단위가 행위(Bobcoin)이므로 코인이 차감되는 단위가 그대로 원가 분해 단위가 된다. Bobalytics는 워크스페이스·팀·사용자 세 층으로 나뉘고, 도입률·Bob factor(Bob이 작성한 커밋 라인 비율)·Bobcoin 지출 세 KPI로 성과와 비용을 나란히 본다.',
        '관찰에서 통제로 넘어가는 지점이 있다. 팀 생성 시 예산 상한을 걸고, 개인별 지출 한도를 관리자가 설정한다. 사용량 기반 과금에서 청구서를 받고 나서 아는 상황을 피할 수 있다.',
        'Claude도 분석 API 두 종과 지출 알림을 갖췄으므로 "대시보드가 있다"로는 갈리지 않는다. Bob의 차이는 분해 단위와 사전 한도이며, Bob factor처럼 기여도를 비용과 함께 보는 지표는 다른 도구에 대응물이 없다.',
      ],
      caveat:
        'Bobalytics는 Enterprise 플랜 전용이다. 그리고 대시보드가 있다는 것과 우리 부서별 배부 기준에 맞는다는 것은 다른 문제다. 반대로 과금 구조 자체는 Bob이 뒤지는 항목이므로(아래 참조) 이 강점을 "비용이 싸다"로 읽으면 안 된다.',
    },
    {
      title: '멀티벤더 라우팅으로 단가를 낮춘다',
      summary: '작업 성격에 따라 모델을 자동 선택 — 단, 공식 문서에 근거가 없다',
      evidence: ['model-choice'],
      body: [
        'IBM은 Claude, Mistral·Devstral, Granite를 작업 성격에 따라 자동 선택하고 정확도·지연시간·비용 규칙 엔진이 라우팅을 결정한다고 발표했다. 절감 폭은 최대 40%로 제시된다. 개발자가 모델을 고민하지 않아도 단가가 내려간다면 조직 단위에서는 분명한 이득이다.',
        '다만 이 강점은 이 자료에서 근거가 가장 약하다. 공식 제품 문서에는 모델 라우팅도, 모델 목록도, 모델 선택 UI도 나오지 않는다. Bobcoins 문서만 "Bob이 모델 사용량을 코인으로 환산한다"고 적는다.',
      ],
      caveat:
        '문서가 침묵한다는 사실 자체가 판단 재료다. 개발자도 조직도 어느 모델이 쓰였는지 확인할 문서화된 수단이 없어 품질 회귀를 추적할 때 변수가 하나 늘어난다. 그리고 Bob의 프런티어 모델이 Claude이므로 라우팅이 절감하는 것은 "Claude를 덜 쓰는 것"이다. 40%는 그 교환의 결과이며 품질 영향은 별도로 재야 한다.',
    },
  ],

  weaknesses: [
    {
      title: '개발자 책상을 벗어나지 못한다',
      summary: '표면 2개. 예약 실행·클라우드 위임·비개발자 접근이 확인되지 않음',
      evidence: ['surfaces', 'async', 'scheduling', 'teamwork'],
      body: [
        'Bob은 독립 애플리케이션으로 설치되는 IDE와 Bob Shell 두 표면이다. Claude는 터미널·VS Code·JetBrains·데스크톱·웹·모바일·Slack·GitHub Actions, Codex는 일곱 표면, Cursor는 다섯 표면을 제공한다.',
        '더 큰 차이는 시간이다. 경쟁 도구 셋은 예약 실행(cron·이벤트 트리거)과 클라우드 에이전트로 사람이 없는 시간에도 일한다. Bob에서 이에 해당하는 기능은 공식 docs 52개 페이지에서 한 건도 나오지 않았다.',
        '이 제약이 Bob의 강점과 정면으로 부딪히는 지점이 있다. 보안 스캔과 코드 품질 점검은 상시 반복이 본질인 작업인데, 경쟁 도구가 이를 야간 예약으로 돌리는 동안 Bob은 개발자가 /review를 실행할 때만 돈다. 대규모 현대화에서도 수십 개 모듈을 동시에 돌리려면 로컬 IDE 한 대가 병목이 된다.',
        'IBM은 Bob을 13개 직무가 참여하는 조직 단위 SDLC 파트너로 제시하지만, 확인된 접근 경로는 개발자용 IDE와 터미널뿐이다. 기획·QA·운영까지 넓히려는 계획이라면 이 행이 결론을 바꾼다.',
      ],
      caveat:
        '예약 실행·비동기 위임·비개발자 표면은 `확인 필요`로 비워 두었다. 문서에 없다는 것이 기능이 없다는 뜻은 아니므로 IBM에 직접 확인해야 한다. 사내 빌드에 해당 기능이 있다면 이 약점의 상당 부분이 사라진다.',
    },
    {
      title: '에이전트 행위를 조직이 강제할 수 없다',
      summary: '통제 대상이 좌석·예산이며, 행위 정책과 행위 감사 로그가 없음',
      evidence: ['policy-control', 'auditability'],
      body: [
        'Bob의 관리자 기능은 잘 갖춰져 있다. SAML IdP를 도메인 단위로 연결하고 DNS TXT로 소유를 검증한 뒤 SSO를 강제하며, 팀 예산과 개인 지출 한도, 프리미엄 패키지 접근을 통제한다.',
        '그러나 통제 대상이 "누가 쓰고 얼마를 쓰는가"이고 "에이전트가 무엇을 해도 되는가"가 아니다. 관리자가 배포하고 개인이 재정의할 수 없는 설정에 해당하는 기능이 문서에 없다. 규칙과 모드는 공유 저장소를 팀원이 각자 clone하는 관례로 배포되고, 자동 승인은 채팅 위 툴바에서 개인이 켜는 설정이다. Claude는 MDM으로 배포한 설정을 사용자가 재정의할 수 없고 Codex는 managed configuration을 제공한다.',
        '감사 쪽도 범위가 다르다. Bob의 Enterprise 활동 로그는 CADF 표준 JSON이라는 좋은 형식을 갖췄지만 기록 대상이 인증(로그인·토큰 갱신)과 관리 활동(사용자·팀·좌석 변경)뿐이다. Codex는 프롬프트·도구 승인 결정·도구 실행 결과·프록시 차단까지 내보낸다. AI가 만든 코드에 문제가 생겼을 때 감사인이 묻는 것은 로그인 시각이 아니라 무엇이 실행됐는지다.',
        '실행 격리는 Bob Shell에 실재한다 — Seatbelt 프로파일과 컨테이너 격리. 다만 IDE에는 없고 기본값이 꺼져 있다. Codex처럼 격리가 기본 동작인 경우와 달리 켜야 하며, 안 켜면 통제가 승인 다이얼로그 한 층으로 돌아간다.',
      ],
      caveat:
        '"Bob은 승인 체크포인트로 통제된다"는 설명은 개발자가 그 체크포인트를 켜 둔다는 가정 위에서만 성립한다. 차선책은 프로젝트 settings.json을 표준화해 저장소에 커밋하는 것이며, 이는 강제가 아니라 합의에 의존한다. 규제 대응이 도입 명분이라면 이 항목을 IBM에 먼저 물어야 한다.',
    },
    {
      title: '자동화의 상한이 셸 호출이다',
      summary: '훅과 SDK가 없어 규칙을 코드로 강제하거나 사내 도구에 넣을 수 없음',
      evidence: ['extensibility', 'embedding', 'delivery'],
      body: [
        '확장 수단 자체는 갖췄다. 커스텀 규칙을 전역·워크스페이스로 나눠 지정하고, 프로젝트 규칙은 코드와 함께 버전 관리되어 clone하면 전파되며 변경이 코드 리뷰를 거친다. Skills로 체크리스트·템플릿을 포함한 워크플로를 정의하고, 커스텀 모드로 도구 접근과 파일 권한을 묶는다. 이 영역에서 Bob은 크게 뒤지지 않는다.',
        '차이는 훅 하나다. 나머지 셋은 편집·커밋·승인 이벤트에 스크립트를 걸어 규칙 위반을 차단할 수 있고, Bob은 규칙을 프롬프트로 전달한다. "조직 표준을 강제한다"가 도입 명분이라면 이 차이가 그 명분의 핵심에 걸린다.',
        '임베딩도 Bob Shell의 비대화형 세션(bob -p)까지다. 에이전트를 우리 코드에 넣는 SDK가 없어 자동화의 형태가 셸 스크립트로 제한된다. Claude와 Codex는 SDK로 오케스트레이션·도구 접근·권한을 코드로 제어한다.',
        'CI 연계도 성숙도 차이가 있다. Bob도 공식 문서가 Bob Shell의 용도로 CI/CD 통합을 명시하므로 못 하는 것은 아니지만, 나머지 셋은 벤더가 관리하는 액션·통합을 제공하고 Bob은 CLI를 직접 부르는 수준이다. 비대화형 세션이 IBMid가 아니라 API 키 인증을 요구하므로 키 발급·보관·회전 절차를 따로 세워야 한다.',
      ],
      caveat:
        '사내 플랫폼 팀이 에이전트를 자체 도구에 넣을 계획이 없다면 임베딩 항목의 가중치는 낮다. 반대로 그 계획이 있다면 이 약점이 결정적이다. 우리 조직의 계획이 어느 쪽인지에 따라 무시해도 되는 항목이 된다.',
    },
    {
      title: '월 비용을 예측하기 어렵다',
      summary: '코인 소모량 변동 + 이월 불가 + 좌석당 지원비 + Premium 별도 계약',
      evidence: ['pricing'],
      body: [
        '선불 크레딧은 상한 관리에는 유리하지만 예산 편성에는 불리하다. 같은 작업의 코인 소모량이 변동하므로 좌석당 월 비용을 고정할 수 없다.',
        '월 할당량은 매월 리셋되며 이월되지 않는다. 사용량을 낮게 잡으면 작업이 끊기고 높게 잡으면 남은 코인이 소멸한다. Overage는 선택 활성화이고 한도를 조절할 수 있으나 한 번 켜면 그 달에는 끌 수 없다.',
        '여기에 좌석마다 지원 비용이 15% 수준으로 더 붙는다(Pro $20+$3, Pro Plus $60+$9, Ultra $200+$30). 그리고 Premium Package가 별도 계약이므로 레거시 현대화를 노리고 도입할수록 총소유비용이 정가에서 멀어진다.',
        'Claude는 좌석 요금에 사용량이 포함되고 초과분은 관리자가 켜야 발생하므로, 네 도구 중 좌석당 월 비용을 사전에 고정하기 가장 쉽다.',
      ],
      caveat:
        '표의 금액은 모두 공개 정가다. 네 도구 모두 대량 계약은 협상 대상이므로 실제 판단은 우리 계약 조건과 예상 작업량으로 다시 계산해야 한다. 이 항목은 금액의 높낮이가 아니라 예측 가능성을 본 것이다.',
    },
    {
      title: 'SaaS 전용이라는 조건이 먼저 걸린다',
      summary: '온프렘은 향후 릴리스이며 시점 미공표',
      evidence: ['deployment'],
      body: [
        'IBM은 GA 시점에 Bob이 SaaS로만 제공되며 온프렘 배포는 데이터 레지던시·규제 요건 대응을 위해 향후 릴리스로 예정돼 있다고 밝혔다. 시점은 공표되지 않았다.',
        'Claude는 Bedrock·Vertex·Foundry를 통해 기업 클라우드 계정 안에서 처리하는 경로와 자체 호스팅 control plane을 제공한다. 코드가 나가는 경계를 조직이 선택할 여지가 가장 넓다.',
        '주의할 점은 이 제약이 다른 항목들과 곱해진다는 것이다. 통제를 근거로 Bob을 고른다는 것은 "SaaS 경계를 받아들이되 그 안에서 통제를 강화한다"는 선택이며, 데이터가 밖으로 나가는 것 자체가 요건 위반인 조직에서는 성립하지 않는다.',
      ],
      caveat:
        '망 분리 환경이 대상이라면 온프렘 시점을 IBM에 먼저 확인하고, 그 답이 나온 뒤에 나머지 항목을 검토하는 것이 순서다. 이 항목에서 막히면 앞의 강점을 검토할 필요가 없어진다.',
    },
  ],

  /* 강점·약점을 조직 조건에 따라 어떻게 읽을지. */
  conditions: {
    title: '우리 조건에서 어떻게 읽을 것인가',
    lead:
      '위 강점과 약점은 조직 조건에 따라 가중치가 크게 달라진다. 같은 표를 보고도 결론이 갈리는 지점을 적어 둔다.',
    rows: [
      {
        when: '메인프레임·IBM i·구형 Java 자산이 많다',
        then: 'Bob이 유력하다. 현대화 전용 워크플로에 대응물이 없어 다른 약점을 상쇄할 수 있다. 단 Premium Package 비용을 별도로 계산하고, 현대화 종료 후에도 쓸 도구인지 따로 판단한다.',
        kind: 'pro',
      },
      {
        when: '토큰 비용이 이미 경영 안건으로 올라와 있다',
        then: 'Bob의 행위 단위 원가와 팀·개인 지출 한도가 값을 낸다. 다만 Enterprise 플랜이 전제이며, 라우팅의 40% 절감은 우리 워크로드로 다시 재야 한다.',
        kind: 'pro',
      },
      {
        when: '개발조직 밖(기획·QA·운영)까지 넓힐 계획이다',
        then: '현재 확인된 범위로는 Bob이 맞지 않는다. 접근 경로가 개발자용 IDE와 터미널뿐이다. IBM에 비개발자 표면 로드맵을 확인한 뒤 판단한다.',
        kind: 'con',
      },
      {
        when: '규제 대응·감사 요건이 도입 명분이다',
        then: '명분과 실물이 어긋날 수 있다. 에이전트 행위를 조직이 강제하는 정책과 행위 감사 로그가 확인되지 않으므로, 파일럿에서 이 두 가지를 직접 확인하기 전에는 근거로 쓰지 않는 편이 안전하다.',
        kind: 'con',
      },
      {
        when: '망 분리 환경이거나 데이터 반출이 금지돼 있다',
        then: '현시점에서는 검토가 여기서 끝난다. 온프렘 시점을 IBM에 먼저 확인해야 한다.',
        kind: 'con',
      },
      {
        when: '사내 플랫폼에 에이전트를 부품으로 넣을 계획이다',
        then: 'SDK가 없어 자동화 형태가 셸 호출로 제한된다. 이 계획이 핵심이라면 Claude나 Codex가 맞다.',
        kind: 'con',
      },
    ],
  },
